...
contract DeedHashedV2 is ERC721, AccessControl {
    ...
    event TokenMinted(
        ...
        DeedHashedStates.TokenState indexed tokenState,
    ...
    bytes32 public constant STATE_UPDATER_ROLE =
        keccak256("STATE_UPDATER_ROLE"); // can manage state of tokens -> 0x7f496d3b3a5b8d5d66b1301ac9407fb7ebb241c9fb60310446582db629b01709
    bytes32 public constant METADATA_LOCKER_ROLE =
        keccak256("METADATA_LOCKER_ROLE"); // can lock metadata -> 0x0af1a227e20c738dadfc76971d0d110fd4b320a2b47db610f169242cda7cbd7e
    bytes32 public constant TOKEN_URI_UPDATER_ROLE =
        keccak256("TOKEN_URI_UPDATER_ROLE"); // can update tokenURI -> 0xd610886bde7b9b3561f4ecdece11096467246c56f3a9958246e8d8b56500f923
    ...
    constructor(
        ...
        _grantRole(MINTER_ROLE, _roleAdmin);
        _grantRole(TRANSFERRER_ROLE, _roleAdmin);
        _grantRole(STATE_UPDATER_ROLE, _roleAdmin);
        _grantRole(METADATA_LOCKER_ROLE, _roleAdmin);
        _grantRole(TOKEN_URI_UPDATER_ROLE, _roleAdmin);
    }

    modifier onlyTransferrer() {
        require(
            hasRole(TRANSFERRER_ROLE, msg.sender),
            "CONTACT_PROPY_FOR_TRANSFER"
        );
        _;
    }
    ...
    modifier onlyTokenURIUpdater() {
        require(
            hasRole(TOKEN_URI_UPDATER_ROLE, msg.sender),
            "NOT_TOKEN_URI_UPDATER"
        );
        _;
    }
    ...
    modifier onlyMetadataLocker() {
        require(
            hasRole(METADATA_LOCKER_ROLE, msg.sender),
            "NOT_METADATA_LOCKER"
        );
        _;
    }
    ...
    function lockMetadata(uint256 _tokenId) public onlyMetadataLocker {
        require(_exists(_tokenId), "INVALID_TOKEN_ID");
        Token storage token = tokens[_tokenId];
    ...
    function unlockMetadata(uint256 _tokenId) public {
